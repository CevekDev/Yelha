...
export async function GET(
  ...
  const connection = await prisma.connection.findFirst({
    where: { id: params.connectionId, platform: 'FACEBOOK', isActive: true },
    select: { messengerVerifyToken: true },
  });

  if (!connection?.messengerVerifyToken) return new Response('Not found', { status: 404 });

  const storedToken = decrypt(connection.messengerVerifyToken);
...
export async function POST(
  ...
  const senderPsid: string = messaging.sender?.id;
  const text: string = messaging.message?.text || '';
  if (!senderPsid || !text) return NextResponse.json({ ok: true });

  const contactId = senderPsid;
  const contactName: string | null = null; // Messenger doesn't provide name in webhook
  ...
  if (!connection || !connection.messengerAccessToken) return NextResponse.json({ ok: true });
  if (connection.isSuspended) return NextResponse.json({ ok: true });

  const accessToken = decrypt(connection.messengerAccessToken);
      ...
      if (pendingOrder) {
        ...
        if (isYes) {
          ...
          if (autoShip && ecoEnabled && pendingOrder.ecotrackTracking) {
            ...
            } catch (e) { console.error('[Messenger] Auto-ship error', e); }
          }
          if (!replyMsg) {
          ...
          if (pendingOrder.ecotrackTracking && ecoEnabled) {
            ...
            } catch (e) { console.error('[Messenger] Ecotrack delete error', e); }
          }
        }
  ...
  if (!responseText) {
    ...
    const systemPrompt = await buildFbSystemPrompt(connection, buildContactContextString(contactCtx), isFirstMessage, ecoDeliveryFee);
    ...
    if (rawResponse.startsWith('[HORS_SUJET]')) {
      ...
      } catch (e) { console.error('[Messenger] Cancellation error', e); }
      ...
      if (tagEnd > jsonStart) {
        ...
        try { await updateOrCreateOrder(connection, contactId, contactName, JSON.parse(rawResponse.slice(jsonStart, tagEnd + 1))); } catch (e) { console.error('[Messenger] Order modify error', e); }
        ...
        if (tagEnd > jsonStart) {
          ...
          try {
            ...
            if (ecoEnabled && newOrderId) {
              ...
              } catch (ecoErr) { console.error('[Messenger][Ecotrack] Error', ecoErr); }
            }
          } catch (e) { console.error('[Messenger] Order parse error', e); }
        } else { responseText = rawResponse; }
    ...
    if (responseText.includes('[ORDER_STATUS_QUERY]')) {
      ...
      } catch (e) { console.error('[Messenger][ORDER_STATUS] Error', e); }
    }
  }
  ...
  if (tokensRequired > 0 && !user.unlimitedTokens) {
    ...
    await prisma.tokenTransaction.create({ data: { userId: connection.userId, type: 'USAGE', amount: -tokensRequired, balance: newBalance, description: 'Facebook Messenger' } });
    if (newBalance <= 100) {
  ...
  try {
    ...
    await upsertContactContext(connection.id, contactId, { contactName });
  } catch (err) { console.error('[Messenger] Save error', err); }

  return NextResponse.json({ ok: true });
}

async function buildFbSystemPrompt(connection: any, contactContext: string, isFirstMessage: boolean, deliveryFee = 0): Promise<string> {
