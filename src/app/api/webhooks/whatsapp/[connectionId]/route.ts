...
export async function GET(
  ...
  const connection = await prisma.connection.findFirst({
    where: { id: params.connectionId, platform: 'WHATSAPP', isActive: true },
    select: { whatsappVerifyToken: true },
  });

  if (!connection?.whatsappVerifyToken) return new Response('Not found', { status: 404 });

  const storedToken = decrypt(connection.whatsappVerifyToken);
...
export async function POST(
  ...
  if (change?.field !== 'messages') return NextResponse.json({ ok: true });

  const value = change?.value;
  if (!value) return NextResponse.json({ ok: true });
  if (value.statuses) return NextResponse.json({ ok: true }); // Skip delivery/read status updates
  ...
  const senderPhone: string = message.from; // e.g. "213XXXXXXXXX"
  ...
  if (!connection || !connection.whatsappPhoneNumberId || !connection.whatsappAccessToken) {
    return NextResponse.json({ ok: true });
  }
  if (connection.isSuspended) return NextResponse.json({ ok: true });

  const phoneNumberId = connection.whatsappPhoneNumberId;
  const accessToken = decrypt(connection.whatsappAccessToken);

  async function sendMessage(txt: string) {
    await sendWhatsAppMessage(phoneNumberId, accessToken, contactId, txt);
  }
      ...
      if (pendingOrder) {
        ...
        if (isYes) {
          ...
          if (autoShip && ecoEnabled && pendingOrder.ecotrackTracking) {
            ...
            } catch (e) { console.error('[WhatsApp] Auto-ship error', e); }
          }
          if (!replyMsg) {
          ...
          if (pendingOrder.ecotrackTracking && ecoEnabled) {
            ...
            } catch (e) { console.error('[WhatsApp] Ecotrack delete error', e); }
          }
        }
  ...
  if (!responseText) {
    ...
    const systemPrompt = await buildWaSystemPrompt(connection, buildContactContextString(contactCtx), isFirstMessage, ecoDeliveryFee);
    ...
    if (rawResponse.startsWith('[HORS_SUJET]')) {
      ...
      if (tagEnd > jsonStart) {
        ...
        } catch (e) { console.error('[WhatsApp] Order modify error', e); }
          ...
          try {
            ...
            if (ecoEnabled && newOrderId) {
              try {
                ...
                const contactCtxFresh = await prisma.contactContext.findUnique({
                  where: { connectionId_contactId: { connectionId: connection.id, contactId } },
                });
                if (found) {
                  const newState: EcotrackState = {
                    step: 'awaiting_delivery_type', orderId: newOrderId, orderData,
                    wilayaId: found.wilayaId, wilayaName: found.wilayaName,
                    communeName: found.communeName, codePostal: found.codePostal, hasStopDesk: found.hasStopDesk,
                  };
                  const currMeta = (contactCtxFresh?.metadata as Record<string, any> | null) ?? {};
                  ...
                  const newState: EcotrackState = {
                    step: 'awaiting_location_confirm', orderId: newOrderId, orderData,
                    wilayaId: suggestions[0].wilayaId, wilayaName: suggestions[0].wilayaName,
                    communeName: suggestions[0].communeName, codePostal: suggestions[0].codePostal,
                    hasStopDesk: suggestions[0].hasStopDesk, suggestions,
                  };
                  const currMeta = (contactCtxFresh?.metadata as Record<string, any> | null) ?? {};
                  await upsertContactContext(connection.id, contactId, { contactName, metadata: { ...currMeta, ecotrackState: newState } });
                  responseText = buildLocationSuggestionsMsg(suggestions, orderData.commune || '', orderData.wilaya || '');
                }
              } catch (ecoErr) { console.error('[WhatsApp][Ecotrack] Error', ecoErr); }
            }
          } catch (e) { console.error('[WhatsApp] Order parse error', e); }
        } else { responseText = rawResponse; }
    ...
    if (responseText.includes('[ORDER_STATUS_QUERY]')) {
      ...
      try {
        ...
        if (latestOrder) {
          const statusLabels: Record<string, string> = {
            PENDING: '⏳ En attente de confirmation', CONFIRMED: '✅ Confirmée',
            PROCESSING: '🔄 En traitement', SHIPPED: '🚚 Expédiée',
            DELIVERED: '📦 Livrée', CANCELLED: '❌ Annulée', RETURNED: '↩️ Retournée',
          };
      ...
      } catch (e) { console.error('[WhatsApp][ORDER_STATUS] Error', e); }
    }
  }

  if (!responseText) return NextResponse.json({ ok: true });

  // ── Facturer & débiter tokens ──────────────────────────────────────────────
  ...
  if (tokensRequired > 0 && !user.unlimitedTokens) {
    ...
    await prisma.tokenTransaction.create({
      data: { userId: connection.userId, type: 'USAGE', amount: -tokensRequired, balance: newBalance, description: 'WhatsApp DM' },
    });
    if (newBalance <= 100) {
  ...
  try {
    ...
    await upsertContactContext(connection.id, contactId, { contactName });
  } catch (err) { console.error('[WhatsApp] Save error', err); }

  return NextResponse.json({ ok: true });
}

// ── System prompt ──────────────────────────────────────────────────────────────
async function buildWaSystemPrompt(connection: any, contactContext: string, isFirstMessage: boolean, deliveryFee = 0): Promise<string> {
  ...
  const productDetailsStr = products.filter((p: any) => p.description)
...
async function saveOrderFromBot(connection: any, contactId: string, contactName: string | null, data: any): Promise<string> {
  ...
  const order = await prisma.order.create({
    data: {
      ...
      contactName: fullName, contactId, contactPhone: data.telephone || null,
