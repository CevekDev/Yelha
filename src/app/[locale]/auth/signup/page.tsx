...
export default function SignUpPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (!res.ok) {
        toast({ title: tCommon('error'), description: json.error || t('errors.emailExists'), variant: 'destructive' });
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <div className="text-3xl font-bold text-primary mb-2">AiReply</div>
        ...
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex items-start gap-2">
              ...
              <Label htmlFor="acceptTerms" className="text-sm font-normal cursor-pointer">{t('acceptTerms')}</Label>
            </div>
