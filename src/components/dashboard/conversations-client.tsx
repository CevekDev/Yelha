...
import { MessageSquare, Bot, Search, User, Send, Plug, RefreshCw, MapPin, FileText, Edit2, Check, X, AlertTriangle, PauseCircle, PlayCircle, Trash2, ChevronLeft } from 'lucide-react';
    ...
    <div className="flex flex-col gap-5">
      ...
      {bulkMode && selectedConvIds.size > 0 && (
        <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-red-500/10 border border-red-500/20">
          <span className="font-mono text-xs text-red-400">{selectedConvIds.size} sélectionnée(s)</span>
          <button onClick={handleBulkDelete} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-500/20 text-red-400 font-mono text-xs hover:bg-red-500/30 transition-all">
            <Trash2 className="w-3.5 h-3.5" /> Supprimer
          </button>
          <button onClick={() => { setBulkMode(false); setSelectedConvIds(new Set()); }} className="ml-auto font-mono text-xs text-white/30 hover:text-white/60">Annuler</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-12 gap-4 h-[600px]">
        ...
        <div className={`col-span-3 rounded-2xl border border-white/[0.06] bg-white/[0.02] flex flex-col overflow-hidden ${mobileView === 'thread' ? 'hidden lg:flex' : 'flex'} lg:flex`}>
          <div className="p-3 border-b border-white/[0.06] space-y-2">
            ...
            <button
              onClick={() => { setBulkMode(!bulkMode); setSelectedConvIds(new Set()); }}
              className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-lg font-mono text-[10px] transition-all"
              style={bulkMode ? { background: '#EF444415', color: '#EF4444' } : { color: 'rgba(255,255,255,0.25)', border: '1px solid rgba(255,255,255,0.06)' }}
            >
              <Trash2 className="w-3 h-3" />
              {bulkMode ? 'Annuler sélection' : 'Sélectionner / Supprimer'}
            </button>
          </div>
                  ...
                  >
                    {bulkMode && (
                      <input
                        ...
                        checked={isSelected}
                        onChange={() => {
                          const s = new Set(selectedConvIds);
                          isSelected ? s.delete(conv.id) : s.add(conv.id);
                          setSelectedConvIds(s);
                        }}
                    ...
                    {!bulkMode && (
                      <button
                        onClick={() => { if (confirm('Supprimer cette conversation ?')) handleDeleteConv(conv.id); }}
                        disabled={deletingId === conv.id}
                        className="p-1.5 rounded-lg text-white/10 hover:text-red-400 hover:bg-red-500/10 transition-all flex-shrink-0 mt-1"
                      >
                        {deletingId === conv.id
            ...
            <>
              ...
              <div className="p-4 border-b border-white/[0.06] flex items-center gap-3">
                <button
                  ...
                  className="lg:hidden p-1.5 rounded-lg text-white/40 hover:text-white hover:bg-white/[0.06] transition-all flex-shrink-0"
                >
